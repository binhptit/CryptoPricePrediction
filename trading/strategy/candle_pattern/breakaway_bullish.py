...
class BreakawayBullish(BasePattern):
    """
    Class for an BreakawayBullish pattern.
    """

    def __init__(self, candlesticks: List[CandleStick]):
        """
        Initializes the BreakawayBullish pattern.
        """
        super().__init__(candlesticks)
        self.candlesticks = candlesticks
        self.trend = "bullish"
        self.pattern_name = "BreakawayBullish"
        self.no_candles = 5

    def run(self):
        """
        Runs the BreakawayBullish pattern.

        Return:
            list[int]: index of the candlestick where the BreakawayBullish pattern was found
        """
        assert len(self.candlesticks) > 1, "There must be at least 2 candlesticks to run the BreakawayBullish pattern"
        found_positions = []
        ...
        for i in range(len(self.candlesticks) - 4):
            ...
            # Second day A black candle that has a gap between the two candle
            # bodies, but the shadows can overlap.
            # Third day A candle with a lower close, but the candle can be any color.
            # Fourth day A black candle with a lower close.
            # Last day A tall white candle with a close within the gap between the
            # first two candles. Ignore the shadows on the first two
    ...
    def __repr__(self) -> str:
        return "BreakawayBullish pattern"
