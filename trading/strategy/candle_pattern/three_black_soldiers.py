...
class ThreeBlackSoldiers(BasePattern):
    """
    Class for an ThreeBlackSoldiers pattern.
    """

    def __init__(self, candlesticks: List[CandleStick]):
        """
        Initializes the ThreeBlackSoldiers pattern.
        """
        super().__init__(candlesticks)
        self.candlesticks = candlesticks
        self.trend = "bearish"
        self.pattern_name = "ThreeBlackSoldiers"
        self.no_candles = 3

    def run(self):
        """
        Runs the ThreeBlackSoldiers pattern.

        Return:
            list[int]: index of the candlestick where the ThreeBlackSoldiers pattern was found
        """
        assert len(self.candlesticks) > 1, "There must be at least 2 candlesticks to run the ThreeBlackSoldiers pattern"
        found_positions = []
    ...
    def __repr__(self) -> str:
        return "ThreeBlackSoldiers pattern"
