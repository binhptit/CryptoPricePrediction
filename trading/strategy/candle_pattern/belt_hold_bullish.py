...
class BeltHoldBullish(BasePattern):
    """
    Class for an BeltHoldBullish pattern.
    """

    def __init__(self, candlesticks: List[CandleStick]):
        """
        Initializes the BeltHoldBullish pattern.
        """
        super().__init__(candlesticks)
        self.candlesticks = candlesticks
        self.trend = "bullish"
        self.pattern_name = "BeltHoldBullish"
        self.no_candles = 1

    def run(self):
        """
        Runs the BeltHoldBullish pattern.

        Return:
            list[int]: index of the candlestick where the BeltHoldBullish pattern was found
        """
        assert len(self.candlesticks) > 1, "There must be at least 2 candlesticks to run the BeltHoldBullish pattern"
        found_positions = []
        ...
        for i in range(len(self.candlesticks)):
            ...
            # Configuration A tall white candle with price opening at the low and closing
            # near the high.
            ...
            if candlestick_0.open != candlestick_0.low:
                continue

            if candlestick_0.close < candlestick_0.high - 0.05 * body_0:
                continue
    ...
    def __repr__(self) -> str:
        return "BeltHoldBullish pattern"
