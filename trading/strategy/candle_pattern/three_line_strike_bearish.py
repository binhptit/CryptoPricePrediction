...
class ThreeLineStrikeBearish(BasePattern):
    """
    Class for an ThreeLineStrikeBearish pattern.
    """

    def __init__(self, candlesticks: List[CandleStick]):
        """
        Initializes the ThreeLineStrikeBearish pattern.
        """
        super().__init__(candlesticks)
        self.candlesticks = candlesticks
        self.trend = "bullish"
        self.pattern_name = "ThreeLineStrikeBearish"
        self.no_candles = 4

    def run(self):
        """
        Runs the ThreeLineStrikeBearish pattern.

        Return:
            list[int]: index of the candlestick where the ThreeLineStrikeBearish pattern was found
        """
        assert len(self.candlesticks) > 1, "There must be at least 2 candlesticks to run the ThreeLineStrikeBearish pattern"
        found_positions = []
        ...
        for i in range(len(self.candlesticks) - 3):
            ...
            # candlestick_2 must be a bearish candlestick
            if candlestick_2.close >= candlestick_2.open:
                continue

            # candlestick_3 must be a bullish candlestick and cover of body of 3 previous candlesticks
    ...
    def __repr__(self) -> str:
        return "ThreeLineStrikeBearish pattern"
