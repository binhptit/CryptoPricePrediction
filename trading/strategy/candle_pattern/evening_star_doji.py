...
class EveningStarDoji(BasePattern):
    """
    Class for an evening star doji pattern.
    """

    def __init__(self, candlesticks: List[CandleStick]):
        """
        Initializes the Evening Star Doji pattern.
        """
    ...
    def run(self):
        """
        Runs the Evening Star Doji pattern.

        Return:
            list[int]: index of the candlestick where the Evening Star Doji pattern was found
        """
        assert len(self.candlesticks) > 2, "There must be at least 3 candlesticks to run the Evening Star Doji pattern"
        found_positions = []
    ...
    def __repr__(self) -> str:
        return "Evening Star Doji Pattern"
